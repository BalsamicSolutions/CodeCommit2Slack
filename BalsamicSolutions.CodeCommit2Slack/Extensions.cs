...
namespace BalsamicSolutions.CodeCommit2Slack
{
    public static class Extensions
    {
        ...
        /// <summary>
        /// loookup a timezone the embeded timezone
        /// chart.
        /// </summary>
        /// <param name="thisStr"></param>
        /// <returns></returns>
        public static TimeZoneInfo TimeZoneById(this string thisStr)
        {
            TimeZoneInfo returnValue = TimeZoneInfo.Utc;
            foreach (TimeZoneInfo tzInfo in TimeZones())
            {
                if (tzInfo.Id.CaseInsensitiveEquals(thisStr))
                {
                    return tzInfo;
                }
            }
        ...
        /// <summary>
        /// gets us a list of all of the timezones embeded in the system
        /// </summary>
        /// <returns></returns>
        public static IReadOnlyCollection<TimeZoneInfo> TimeZones()
        {
            if (null == _TimeZones)
            {
                string jsonTimeZones = null;
                Assembly thisAssembly = Assembly.GetExecutingAssembly();
                string[] resnames = thisAssembly.GetManifestResourceNames();
                using (Stream resourceStream = thisAssembly.GetManifestResourceStream("BalsamicSolutions.CodeCommit2Slack.timeZones.json"))
                {
                    using (StreamReader streamReader = new StreamReader(resourceStream))
                    {
                        jsonTimeZones = streamReader.ReadToEnd();
                        _TimeZones = Newtonsoft.Json.JsonConvert.DeserializeObject<List<TimeZoneInfo>>(jsonTimeZones);
                    }
                }
            }
